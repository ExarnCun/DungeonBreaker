...
/**
 * 
 * A block is a object, that can be rendered by a {@link GameWorld} and has some
 * specific events.<br>
 * Every "dead" visible object, that is not an overlay should be a Block.<br>
 * You can also implement some methods in addition to extending the Block so you
...
public abstract class Block implements Renderable {
	 ...
	 * THIS LOCATION IS NOT THE LOCATION IN PIXELS, THE PIXEL LOCATION IS
	 ...
	 * THIS DIMENSION IS NOT THE DIMENSION IN PIXELS, THE PIXEL DIMENSION IS
	 * CALCULATED LIKE THIS:
	...
	public void Render(Graphics2D g, GameWorld world, Object[] args) {
		g.drawImage(Texture, LocationOnScreen(world.region).x, LocationOnScreen(world.region).y,
				SizeOnScreen(world.region).width, SizeOnScreen(world.region).height, null);
	}
	 ...
	 * @param collider
	 *            The object which entered ( / collided with) this block
	 */
	 ...
	 *            The world this block belongs to
	 * @param entity
	 *            The object which holds the item
	 * @param item
	 *            The item used on this block
	 */
	 ...
	 *            The object which holds the item
	 * @param item
	 *            The item used to break this block
	 */
